...
4. 还原真实最小树形图：从最外层超级节点开始，对于超级节点s，若其入边为(u,s),连接环内的具体边是(a,v),且形成这个有向环时，环内指向这个v的边是(b,v),则将E中的(b,v)删去。不断重复此操作直至所有超级节点被还原。
5. 最小总代价即为所有减去的reduced cost之和。寻找真实路径只需给每个边添加原始起点、终点两个属性。最后遍历E来获取每一个点的承接关系
